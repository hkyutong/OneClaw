...
const buildMissingEntryErrorMessage = async () => {
  ...
  const githubInstallRef =
    CLI_NAME === "oneclaw" ? "github:hkyutong/OneClaw#<ref>" : "github:openclaw/openclaw#<ref>";
  lines.push(
    `For pinned GitHub installs, use \`npm install -g ${githubInstallRef}\` instead of a raw \`/archive/<ref>.tar.gz\` URL.`,
  );
  lines.push(
    `For releases, use \`npm install -g ${CLI_NAME === "oneclaw" ? "oneclaw@latest" : "openclaw@latest"}\`.`,
  );
  return lines.join("\n");
};
