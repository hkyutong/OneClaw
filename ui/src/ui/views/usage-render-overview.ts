  ...
  if (!daily.length) {
    return html`
      <div class="daily-chart-compact">
        <div class="sessions-panel-title">每日使用情况</div>
  ...
  return html`
    <div class="daily-chart-compact">
      <div class="daily-chart-header">
        ...
        <div class="card-title">每日${isTokenMode ? "令牌" : "成本"}使用情况</div>
      </div>
...
function renderCostBreakdownCompact(totals: UsageTotals, mode: "tokens" | "cost") {
  ...
  return html`
    <div class="cost-breakdown cost-breakdown-compact">
      <div class="cost-breakdown-header">${isTokenMode ? "令牌" : "成本"}按类型分布</div>
...
function renderUsageInsights(
  ...
  const throughputHint = "吞吐量表示活跃时间内的每分钟令牌数，越高越好。";
  const tokensHint = "当前范围内每条消息的平均令牌数。";
  const costHint = showCostHint
    ? "当提供商上报成本时，这里显示每条消息的平均成本。当前范围内部分或全部会话缺少成本数据。"
    : "当提供商上报成本时，这里显示每条消息的平均成本。";
  ...
  const topModels = aggregates.byModel.slice(0, 5).map((entry) => ({
    ...
    sub: `${formatTokens(entry.totals.totalTokens)} 令牌 · ${entry.count} 条消息`,
  }));
      ...
      <div class="usage-summary-grid">
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            消息
            <span class="usage-summary-hint" title="当前范围内的用户与助手消息总数。">?</span>
          </div>
          <div class="usage-summary-value">${aggregates.messages.total}</div>
          <div class="usage-summary-sub">
            ${aggregates.messages.user} 条用户消息 · ${aggregates.messages.assistant} 条助手消息
          </div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            工具调用
            <span class="usage-summary-hint" title="当前范围内跨会话的工具调用总数。">?</span>
          </div>
          <div class="usage-summary-value">${aggregates.tools.totalCalls}</div>
          <div class="usage-summary-sub">共使用 ${aggregates.tools.uniqueTools} 个工具</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            错误
            <span class="usage-summary-hint" title="当前范围内的消息或工具错误总数。">?</span>
          </div>
        ...
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            会话
            <span class="usage-summary-hint" title="当前范围内的不重复会话数。">?</span>
          </div>
          <div class="usage-summary-value">${sessionCount}</div>
          <div class="usage-summary-sub">当前范围内共 ${totalSessions} 个</div>
        </div>
        <div class="usage-summary-card">
          <div class="usage-summary-title">
            吞吐量
        ...
        <div class="usage-summary-card">
          ...
          <div class="usage-summary-sub">
            ${formatTokens(totals.cacheRead)} 缓存读取 · ${formatTokens(cacheBase)} 提示输入
          </div>
        </div>
      </div>
      <div class="usage-insights-grid">
        ${renderInsightList("高频模型", topModels, "暂无模型数据")}
        ${renderInsightList("高频供应商", topProviders, "暂无供应商数据")}
        ${renderInsightList("高频工具", topTools, "暂无工具调用")}
        ${renderInsightList("高频代理", topAgents, "暂无代理数据")}
        ${renderInsightList("高频频道", topChannels, "暂无频道数据")}
...
function renderSessionsCard(
  ...
  const buildSessionMeta = (s: UsageSessionEntry): string[] => {
    ...
    if (showColumn("channel") && s.channel) {
      parts.push(`频道：${s.channel}`);
    }
    if (showColumn("agent") && s.agentId) {
      parts.push(`代理：${s.agentId}`);
    }
    if (showColumn("provider") && (s.modelProvider || s.providerOverride)) {
      parts.push(`提供商：${s.modelProvider ?? s.providerOverride}`);
    }
    if (showColumn("model") && s.model) {
      parts.push(`模型：${s.model}`);
    }
    if (showColumn("messages") && s.usage?.messageCounts) {
      parts.push(`消息：${s.usage.messageCounts.total}`);
    }
    if (showColumn("tools") && s.usage?.toolUsage) {
      parts.push(`工具：${s.usage.toolUsage.totalCalls}`);
    }
    if (showColumn("errors") && s.usage?.messageCounts) {
      parts.push(`错误：${s.usage.messageCounts.errors}`);
    }
    if (showColumn("duration") && s.usage?.durationMs) {
      parts.push(`时长：${formatDurationCompact(s.usage.durationMs, { spaced: true }) ?? "—"}`);
    }
    return parts;
  };
          ...
          : sessions.length === 0
            ...
            : html`
	                <div class="session-bars">
	                  ...
	                  ${sessions.length > 50 ? html`<div class="muted" style="padding: 8px; text-align: center; font-size: 11px;">另有 ${sessions.length - 50} 条</div>` : nothing}
	                </div>
	              `
      }
      ${
